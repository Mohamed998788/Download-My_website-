// التعرف التلقائي على أبعاد وخصائص الشاشة
        ...
        function generateRandomSensitivity() {
            ...
            // تحديد النطاق بناءً على السرعة المختارة بشكل دقيق - قيم محسنة للتحديث الجديد
            let baseMinValue, baseMaxValue;
            
            if (speed === "fast") {
                // سريعة - نطاق مناسب للسرعة العالية، مع زيادة الحد الأدنى
                baseMinValue = 160;
                baseMaxValue = 200;
            } else if (speed === "medium") {
                // متوسطة - نطاق متوسط لتناسب معظم اللاعبين، مع زيادة القيم
                baseMinValue = 120;
                baseMaxValue = 160;
            } else if (speed === "slow") {
                // بطيئة - نطاق أقل للتحكم الدقيق، مع زيادة طفيفة للقيم
                baseMinValue = 70;
                baseMaxValue = 110;
            }
            
            // تطبيق عوامل تصحيح للأجهزة المختلفة بشكل معتدل
            // زيادة عامل التصحيح للأجهزة لإعطاء استجابة أفضل
            const deviceAdjustmentFactor = (dpiMultiplier * screenSizeMultiplier - 1.0) * 0.4 + 1.05;
            ...
            baseMinValue = Math.min(Math.max(baseMinValue, 20), 190);
            baseMaxValue = Math.min(Math.max(baseMaxValue, baseMinValue + 15), 200);
            
            // عامل أبعاد الشاشة محسّن - أفضل للنسبة 16:9
            const aspectRatioMultiplier = aspectRatio >= 1.7 && aspectRatio <= 1.8 ? 1.05 : 0.98;
            ...
            // حساب معاملات التصحيح لكل نوع من الحساسيات - محسّنة للتحديث الجديد
            let generalMult, redDotMult, scope2xMult, scope4xMult, sniperMult, freeLookMult;
            
            if (speed === "fast") {
                // سريعة - نسب مناسبة للسرعة العالية مع زيادة قيم المضاعفات
                generalMult = 1.0;    // حساسية عامة عالية
                redDotMult = 0.98;    // تقليل طفيف جداً للنقطة الحمراء
                scope2xMult = 0.95;   // تقليل طفيف للسكوب 2x
                scope4xMult = 0.88;   // تقليل طفيف للسكوب 4x
                sniperMult = 0.75;    // تقليل متوسط للقناصة
                freeLookMult = 1.08;  // زيادة للنظر الحر
            } else if (speed === "medium") {
                // متوسطة - توازن بين السرعة والدقة مع تحسين القيم
                generalMult = 1.0;    // حساسية عامة متوازنة
                redDotMult = 0.96;    // تقليل طفيف للنقطة الحمراء
                scope2xMult = 0.89;   // تقليل طفيف للسكوب 2x
                scope4xMult = 0.79;   // تقليل معتدل للسكوب 4x
                sniperMult = 0.65;    // تقليل متوسط للقناصة
                freeLookMult = 1.05;  // زيادة للنظر الحر
            } else { // slow
                // بطيئة - تركيز على الدقة مع تحسين القيم
                generalMult = 1.0;    // حساسية عامة منخفضة
                redDotMult = 0.94;    // تقليل طفيف للنقطة الحمراء
                scope2xMult = 0.84;   // تقليل متوسط للسكوب 2x
                scope4xMult = 0.74;   // تقليل متوسط للسكوب 4x 
                sniperMult = 0.55;    // تقليل كبير للقناصة للدقة العالية
                freeLookMult = 1.04;  // زيادة طفيفة للنظر الحر
            }
            
            // إضافة عنصر عشوائي صغير لتنويع النتائج مع الحفاظ على النطاق المحدد - تقليل التنوع
            const randomFactor = 0.97 + (Math.random() * 0.06); // 0.97-1.03 للحصول على توازن أفضل
            
            // حساب الحساسيات مع مراعاة عوامل التصحيح والعوامل الفنية
            sensitivities.general = Math.min(200, Math.max(40, Math.round(baseValue * generalMult * aspectRatioMultiplier * randomFactor)));
            sensitivities.redDot = Math.min(200, Math.max(35, Math.round(baseValue * redDotMult * aspectRatioMultiplier * randomFactor)));
            sensitivities.scope2x = Math.min(200, Math.max(30, Math.round(baseValue * scope2xMult * aspectRatioMultiplier * randomFactor)));
            sensitivities.scope4x = Math.min(200, Math.max(25, Math.round(baseValue * scope4xMult * aspectRatioMultiplier * randomFactor)));
            sensitivities.sniper = Math.min(200, Math.max(20, Math.round(baseValue * sniperMult * aspectRatioMultiplier * randomFactor)));
            sensitivities.freeLook = Math.min(200, Math.max(45, Math.round(baseValue * freeLookMult * aspectRatioMultiplier * randomFactor)));
